...
if not MAP_FILE.exists():
    print(f"[async-flame] ERROR: future_map.json not found at {MAP_FILE}. Run future_analyzer/export_map.py first.")
    # Fallback to an empty map if not found, to allow GDB script to load
    FUT_MAP = {}
else:
...
# ---------- load runtime plugin ----------
try:
    plugin_mod_path = f"runtime_plugins.{PLUGIN_NAME}"
    base_plugin_mod_path = "runtime_plugins.base"
    
    # Ensure GDB can find the plugin directory
    gdb_script_dir = str(pathlib.Path(__file__).resolve().parent)
    if gdb_script_dir not in sys.path:
    ...
    print(f"[async-flame] Loaded runtime plugin: {plugin.name}")
    ...
    if gdb_script_dir not in sys.path:
        sys.path.insert(0, gdb_script_dir)
    plugin = importlib.import_module(base_plugin_mod_path).RuntimePlugin()
...
class DumpTrace(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        print(f"[async-flame] {out_file} written (events={len(trace_events)})")
